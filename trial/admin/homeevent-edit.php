...
if (isset($_POST['form1'])) {
    ...
    if (empty($_POST['title']) || empty($_POST['genre']) || empty($_POST['type']) || empty($_POST['location']) || empty($_POST['date_range']) || empty($_POST['rating']) || empty($_POST['modal_id']) || empty($_POST['link'])) {
        $valid = 0;
        $error_message .= 'All fields are required except the image.<br>';
    }

    // Ensure the directory exists
    $upload_dir = __DIR__ . '/../uploads/homeevent/';
    ...
    if ($valid == 1) {
        if (isset($_REQUEST['id'])) {
            ...
            $statement = $pdo->prepare("UPDATE homeevent SET title=?, genre=?, type=?, location=?, date_range=?, rating=?, image_url=?, modal_id=?, link=? WHERE id=?");
            $statement->execute([
                $_POST['title'],
                $_POST['genre'],
                $_POST['type'],
            ...
            // Insert new record
            $statement = $pdo->prepare("INSERT INTO homeevent (title, genre, type, location, date_range, rating, image_url, modal_id, link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute([
                $_POST['title'],
                $_POST['genre'],
                $_POST['type'],
...
if (isset($_REQUEST['id'])) {
    ...
    $title = $genre = $type = $location = $date_range = $rating = $image_url = $modal_id = $link = '';
}
?>
                ...
                <div class="box box-info">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="text" class="form-control" name="date_range" value="<?php echo htmlspecialchars($date_range); ?>" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="col-sm-2 control-label">Rating <span>*</span></label>
                            <div class="col-sm-6">
                                <input type="number" step="0.1" class="form-control" name="rating" value="<?php echo htmlspecialchars($rating); ?>" required>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                ...
                                <?php if (!empty($image_url)): ?>
                                    <img src="../uploads/homeevent/<?php echo htmlspecialchars($image_url); ?>" alt="Event Image" style="width:150px; margin-top:10px;">
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="col-sm-2 control-label">Modal ID <span>*</span></label>
                            <div class="col-sm-6">
                                <input type="text" class="form-control" name="modal_id" value="<?php echo htmlspecialchars($modal_id); ?>" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="col-sm-2 control-label">Link <span>*</span></label>
                            <div class="col-sm-6">
                                <input type="url" class="form-control" name="link" value="<?php echo htmlspecialchars($link); ?>" required>
                            </div>
                        </div>
                        <div class="form-group">
